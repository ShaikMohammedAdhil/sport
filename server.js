...
app.post('/join-event/:id', (req, res) => {
    ...
    db.get('SELECT * FROM player_events WHERE player_id = ? AND event_id = ?', [playerId, eventId], (err, row) => {
        ...
        // Add player to the player_events table
        db.run('INSERT INTO player_events (player_id, event_id) VALUES (?, ?)', [playerId, eventId], (err) => {
            if (err) {
                return res.status(500).send('Error joining event');
            }

            // Update registrations column in events table (add playerId to the list of registrations)
            db.get('SELECT registrations FROM events WHERE id = ?', [eventId], (err, event) => {
                if (err) return res.status(500).send('Error fetching event');

                const registrations = JSON.parse(event.registrations);
                registrations.push(playerId);

                db.run('UPDATE events SET registrations = ? WHERE id = ?', [JSON.stringify(registrations), eventId], (err) => {
                    if (err) return res.status(500).send('Error updating event registrations');
...
app.post('/unjoin-event/:id', (req, res) => {
    ...
    db.get('SELECT * FROM player_events WHERE player_id = ? AND event_id = ?', [playerId, eventId], (err, row) => {
        ...
        // Remove player from the player_events table
        db.run('DELETE FROM player_events WHERE player_id = ? AND event_id = ?', [playerId, eventId], (err) => {
            if (err) {
                return res.status(500).send('Error unjoining event');
            }

            // Update registrations column in events table (remove playerId from the list of registrations)
            db.get('SELECT registrations FROM events WHERE id = ?', [eventId], (err, event) => {
                if (err) return res.status(500).send('Error fetching event');

                let registrations = JSON.parse(event.registrations);
                registrations = registrations.filter(id => id !== playerId);  // Remove the playerId from the list
